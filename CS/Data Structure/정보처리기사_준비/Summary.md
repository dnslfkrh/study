...
# 스택 (Stack)
- 리스트의 한쪽 끝에서만 삽입, 삭제가 진행
- 가장 나중에 추가된 데이터가 가장 먼저 삭제됨 (후입선출: LIFO, Last In Fist Out, 프링글스가 완벽한 예시가 아닐까)
- 함수 호출 제어, 인터럽트의 처리 등에서 응용 가능
- PUSH: 삽입
- POP: 삭제
- PEEK: 맨 위 데이터 확인
- TOP: 가장 마지막에 삽입된 데이터
...
- X: 스택 이름
- Overflow: 꽉 찬 상태
- Uderflow: 스택 포인터가 주소 0이면, 삭제할 게 없다 = Underflow 발생

# 큐 (Queue)
- 한쪽에서는 삽입, 한쪽에서는 삭제
- 선입 선출 (FIFO: First In First Out)
- 프런트 포인터: 가장 먼저 삽입된 자료 기억 공간, 삭제 시 사용
- 리어 포인터: 가장 마지막에 삽입된 자료 기억 공간, 삽입 시 사용

# 데크 (Deque)
- 삽입과 삭제가 동시에 양쪽에서 가능한 자료 구조
- 삭제만 양쪽에서 가능하고 삽입은 한쪽에서만 가능한 `Scroll`
- 삽입만 양쪽에서 가능하고 삭제는 한쪽에서만 가능한 `Shelf`

# 그래프 (Graph)
- `그래프 G`는 정점 V(Vertex)와 간선 E(Edge) 두 집합으로 구성
- 방향성 유무에 따라 방향 그래프와 무방향 그래프로 분리
- 통신망, 교통망, 유기화학 구조식 등에 사용
- 트리는 사이클이 없는 그래프
- 무방향 그래프의 간선 수: 점 개수 * (점 개수 - 1) / 2 (4*3/2 = 6)
- 방향 그래프의 간선 수: 점 개수 * (점 개수 - 1) (4*3 = 12)
